 ...
 */
const STATS = [
  { value: 10.2, label: "M Drained", subtitle: "YieldBlox", prefix: "$" },
...
function Stat({
  ...
  const display = useTransform(spring, (current) => {
    if (value < 10 && value > 0) return current.toFixed(1);
  ...
  return (
    <div ref={ref} className="flex flex-col items-center gap-1">
      <div className="font-mono text-4xl font-medium tabular md:text-5xl">
        <span className="text-[var(--color-text-dim)]">{prefix}</span>
        <motion.span className="text-[var(--color-text)]">{display}</motion.span>
      </div>
